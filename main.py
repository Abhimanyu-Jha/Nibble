...
def funcMerger(restaurant,location):
	...
	zomatoData,zomatoLink = get_zomato_menu(restaurant,location)
	menu={}
	for item in swiggyData:
		try:
			menu[item] = [swiggyData[item][0],[float(swiggyData[item][-1]),zomatoData[item][-1]]]
		except:
			menu[item] = [swiggyData[item][0],[float(swiggyData[item][-1]),"NA"]]


	return json.dumps({'menu':menu,'links':[swiggyLink,zomatoLink]})
